...
export default function ProfilePage({locale='en' as const}:{locale?:'en'|'ko'}) {
  ...
  return <PageShell locale={locale} path="/profile">
    <section className="profile-hero content-width">
      ...
      <Image src={site.assets.profileHero} alt="Taebin Yoo as a child performing" width={478} height={478} priority/>
    </section>
    <div className={`profile-content${locale === 'ko' ? ' profile-content-ko' : ''}`}>
      ...
      <section className="profile-education">{site.profile.educationParagraphs.map(p=><p key={p}>{p}</p>)}</section>
      <section className="profile-media"><YoutubeEmbed/><div><h2>Podcast</h2><a className="text-link" href="https://youtu.be/FkPcCPqDZV0?si=xatYoRNts6ZmmDDR">WATCH THE VIDEO →</a></div></section>
    </div>
  </PageShell>;
}
