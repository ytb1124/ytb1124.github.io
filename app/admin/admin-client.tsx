  ...
  return <main className="admin-shell">
    ...
    {tab === 'production' && <div className="admin-panel">
      ...
      <details className="admin-card" open>
        <summary><span>Selected Projects</span><span>{production.selectedProjects.projects.length} projects</span></summary>
        <div className="admin-card-body">
          <div className="admin-columns"><Field label="English section label" value={production.selectedProjects.eyebrow.en} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.eyebrow.en=value;})}/><Field label="한국어 섹션 이름" value={production.selectedProjects.eyebrow.ko} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.eyebrow.ko=value;})}/></div>
          {production.selectedProjects.projects.map((project:any,index:number)=><section className="admin-subsection" key={project.id}><h3>{project.title}</h3><div className="admin-columns"><Field label="프로젝트 제목" value={project.title} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].title=value;})}/><Field label="연도" value={project.year} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].year=value;})}/><Field label="YouTube URL" value={project.videoUrl} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].videoUrl=value;})}/></div><div className="admin-columns"><section><h3>English</h3><Field label="Category" value={project.category.en} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].category.en=value;})}/><Field label="Question" value={project.question.en} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].question.en=value;})}/><StringList label="Body" values={project.body.en} onChange={(values)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].body.en=values;})}/></section><section><h3>한국어</h3><Field label="분류" value={project.category.ko} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].category.ko=value;})}/><Field label="질문" value={project.question.ko} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].question.ko=value;})}/><StringList label="본문" values={project.body.ko} onChange={(values)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].body.ko=values;})}/></section></div><ImageEditor label="대표 사진" value={project.image} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].image=value;})} queueAsset={queueAsset} removeAsset={removeAsset}/><GalleryEditor values={project.gallery} onChange={(values)=>updateDocument(paths.musicProduction,(doc)=>{doc.selectedProjects.projects[index].gallery=values;})} queueAsset={queueAsset} removeAsset={removeAsset}/></section>)}
        </div>
      </details>
      <details className="admin-card" open>
        ...
        <div className="admin-card-body">
          ...
          <div className="admin-columns"><Field label="섹션 제목" value={production.studio.title} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.studio.title=value;})}/><Field label="앨범 제목" value={production.studio.albumTitle} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.studio.albumTitle=value;})}/><Field label="앨범 정보" value={production.studio.meta} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.studio.meta=value;})}/><Field label="YouTube 앨범 URL" value={production.studio.playlistUrl} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.studio.playlistUrl=value;})}/><Field label="커버 대체 텍스트" value={production.studio.coverAlt} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.studio.coverAlt=value;})}/></div>
      ...
      <details className="admin-card">
        ...
        </div><Field label="YouTube 재생목록 URL" value={production.mixing.playlistUrl} onChange={(value)=>updateDocument(paths.musicProduction,(doc)=>{doc.mixing.playlistUrl=value;})}/></div>
      </details>
      <div className="admin-title-row"><h2>03 · Live Sound 활동</h2><button className="secondary" onClick={()=>updateDocument(paths.activities,(doc)=>doc.unshift({id:crypto.randomUUID(),title:'새 행사',englishTitle:'New Event',eventType:'Live Event Production',year:new Date().getFullYear().toString(),image:'',role:'Mixing Engineer'}))}>행사 추가</button></div>
